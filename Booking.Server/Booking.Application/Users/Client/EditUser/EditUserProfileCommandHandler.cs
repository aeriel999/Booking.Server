...
{
	public async Task<ErrorOr<EditUserProfileCommandResult>> Handle(
		EditUserProfileCommand request, CancellationToken cancellationToken)
	{
		var userOError = await repository.GetUserByIdAsync(request.Id);
