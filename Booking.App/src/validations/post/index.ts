export const CityNameValidator = (value: string): string | undefined => {
    ...
    if (/[£#“”]/.test(value)) return 'City must not contain the following characters: £ # “”';
...
export const StreetNameValidator = (value: string): string | undefined => {
    ...
    if (!/^[A-Za-z\s]+$/.test(value)) return 'Street must contain only letters and spaces';
    if (/[£#“”]/.test(value)) return 'Street must not contain the following characters: £ # “”';
    return undefined; // Return undefined if validation passes
};
...
export const  BuildingNumberValidator = (value: string): string | undefined => {
    if (value.length > 5) return 'Number Of Building must be less than 5 characters long';
    if (!/^[A-Za-z0-9\s]+$/.test(value)) return 'Number Of Building must contain only letters, digits, and spaces';
    if (/[£#“”]/.test(value)) return 'Number Of Building must not contain the following characters: £ # “”';
    return undefined; // Return undefined if validation passes
};

export const NumberOfRoomsValidator = (value: string): string | undefined => {
    if (value.length > 5) return 'Number Of Rooms must be less than 5 characters long';
