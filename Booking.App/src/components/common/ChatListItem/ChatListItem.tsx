...
export const ChatListItem = (info: IChatListItem) => {
    ...
    useEffect(() => {
        if (deletedChatRoom && chats && chats.find((element) => element.chatRoomId == deletedChatRoom)) {
            setChats((chats) =>
                chats!.filter((element) => element.chatRoomId !== deletedChatRoom));

        }
    }, [deletedChatRoom])
    useEffect(() => {
        if (savedChatRoom && chats && chats.find((element) => element.chatRoomId == savedChatRoom)) {
            setIsOpen(true);
        }
    }, [savedChatRoom])
                ...
                <div className="chat" style={{ display: isOpen ? 'inline-block' : 'none', padding: 15, boxSizing: "border-box" }} >
                    {chats ? chats.map((item) => (
                        <div
                            tabIndex={0}
                            key={item.chatRoomId}
                            className="chat-list-lower-item" onClick={() => {
