                ...
                <Grid container spacing={2}>
                    <Grid xs={3}>
                        <CardMedia
                            ...
                            image={`${APP_ENV.BASE_URL}/images/avatars/${realtor.avatar}`}
                            title={realtor.name}
                        />
                    </Grid>
                    <Grid xs={7}>
                        ...
                        >
                            ...
                            <Rating name="half-rating-read" defaultValue={realtor.rating} precision={0.5} readOnly />
                        </Box>
                    </Grid>
                    <Grid item xs={2}>
                        <Stack spacing={3}>
                            {isLogin ? <Button sx={{ height: "50%" }} variant="outlined">SEND MESSAGE</Button> : ""}
                    ...
                    <Grid item xs={12}>
                        ...
                        {posts != null ? <ImageList sx={{ width: "50%", height: 300, margin: "auto" }} cols={posts.length} rowHeight={250}>
                            {posts.map((item) => (
                                <ImageListItem key={item.imagePost} sx={{ border: "1px solid black", margin: "auto" }} >
                                    <img
                                        srcSet={`${APP_ENV.BASE_URL}/images/posts/${item.imagePost}`}
                                        src={`${APP_ENV.BASE_URL}/images/posts/${item.imagePost}`}
                                        alt={item.name}
                                        loading="lazy"
                                        style={{ width: 200, height: 100, objectFit: 'cover', margin: 5 }}
                                    />
                    ...
                    <Accordion sx={{ width: "100%" }}>
                        ...
                        <AccordionDetails>
                            {isLogin ? <> <Box sx={{ display: "flex", columnGap: 5 }}>
                                <CustomizedDialogs
                                    isOpen={isOpen}
                                ...
                                <Button variant="contained" sx={{ marginBottom: 5 }} onClick={sendFeedbackAsync}>ENTER</Button></> : ""}
                            <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
                                {feedbacks != null ? feedbacks.items.$values.map((item) =>

                                    <ListItem alignItems="flex-start" sx={{ border: "1px solid black", padding: 3 }}>
                                        <ListItemText
                                            primary={item.client}
                                            ...
                                            secondary={
                                                <React.Fragment>
                                                    ...
                                                    <Typography>
                                                        {`${new Date(item.feedbackAt).getDate()}-${new Date(item.feedbackAt).getMonth()}-${new Date(item.feedbackAt).getFullYear()}`}
                                                    </Typography>
                                                </React.Fragment>
                                            }
                                        />
                                        <Rating sx={{ marginBottom: "auto", marginTop: "auto" }} name="half-rating-read" defaultValue={item.rating} precision={0.5} readOnly />
                                    </ListItem>
                                ) : ""}

                            </List>
                            <Box sx={{ width: "auto", margin: "auto" }}>
                                {feedbacks != null && feedbacks.totalCount > 10 ? <Pagination page={activePage} count={Math.ceil(feedbacks.totalCount / 10)} onChange={changePage} color="primary" /> : ""}
                            </Box>
                        </AccordionDetails>
                    </Accordion>
                </Grid >
