...
export const HistoryOfFeedbacksPage = () => {
    ...
    const historyOfFeedbacks = useAppSelector(
        (state: RootState) => state.post.historyOfFeedbacksByClient
    );
    ...
    return (
        <div id="history-of-feedbacks">
            ...
            {historyOfFeedbacks
                ? historyOfFeedbacks.items.$values.map((item) => (
                      <div
                          tabIndex={0}
                          ...
                          onClick={() => {
                              ...
                              dispatch(
                                  setIdOfSelectedFeedback(item.feedbackId)
                              );
                          }}
                          onKeyDown={(e) => {
                              if (e.key === "Enter") {
                      ...
                      >
                          <div className="history-of-feedbacks-item-post-details">
                              <img
                                  src={`${APP_ENV.BASE_URL}/uploads/posts/${item.imageOfPost}`}
                                  alt="Image of post"
                              />
            ...
            {historyOfFeedbacks && historyOfFeedbacks.totalCount > 6 ? (
                <Pagination
                    page={currentPage}
                    sizeOfPage={6}
                    countOfPosts={historyOfFeedbacks.totalCount}
